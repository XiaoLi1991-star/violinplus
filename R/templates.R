...
violin_template_examples <- function() {
  rows <- list(
    ...
    list(
      ...
      claim = "Validation cohort retains a visible CRP dose-response pattern.",
      filter_label = "CRP at Week 8 in validation cohort",
      filter = function(data) data$facet == "CRP" & data$pair == "Week 8" & data$cohort == "Validation" & !is.na(data$value),
      x = "group",
      y = "value",
      facet = "",
      subject = "",
      title = "Validation CRP dose response",
      ylab = "CRP (mg/L)"
    ),
    list(
      template = "box_jitter",
    ...
    list(
      template = "facet_grid",
      claim = "The treatment effect is marker-specific across inflammatory and serum markers.",
      filter_label = "Week 8 values for all markers",
      filter = function(data) data$pair == "Week 8" & !is.na(data$value),
      x = "group",
      y = "value",
      facet = "facet",
      subject = "",
      title = "Marker-specific treatment effects",
      ylab = "Normalized biomarker value"
    )
  )
  out <- data.frame(
